...
//! Clase conjunto
 /*! conjunto<T,CMP>::conjunto, conjunto<T,CMP>::find, conjunto<T,CMP>::size,  conjunto<T,CMP>::insert, conjunto<T,CMP>::erase,
 * conjunto<T,CMP>::count, conjunto<T,CMP>::clear, conjunto<T,CMP>::empty, conjunto<T,CMP>::operator=, conjunto<T,CMP>::begin,
 * conjunto<T,CMP>::end, conjunto<T,CMP>::cbegin, conjunto<T,CMP>::cend, conjunto<T,CMP>::lower_bound,
 * conjunto<T,CMP>::upper_bound.
 *
 ...
 * En nuestro caso el conjunto va a tener un subconjunto restringido de métodos (inserción de elementos,
 * consulta de un elemento, etc). Este conjunto "simulará" un conjunto de la stl, con algunas claras diferencias pues,
 * entre otros,  no estará dotado de la capacidad de iterar (recorrer) a través de sus elementos.
 *
 * Asociado al conjunto, tendremos el tipo  \code conjunto::value_type \endcode que permite hacer referencia al elemento
 * almacenados en cada una de las posiciones del conjunto, en nuestro caso mutaciones (SNPs). Es requisito que el tipo
 * conjunto::value_type tenga definida una relación de orden, CMP, y el operador de asignación, operator= .
 *
...
public:
	...
	*/
	...
	@return Si existe una entrada en el conjunto con ese valor devuelve el iterador a su posicion, en caso contrario
	devuelve iterador al final de conjunto
	...
	*/
	...
	/** @brief busca una entrada en el conjunto
	@param[in] s: entrada  a buscar.
	@return Si existe una entrada en el conjunto con ese valor devuelve el iterador constante a su posicion, en caso contrario
	devuelve iterador al final de conjunto
	@post no modifica el conjunto.
	*/
	...
	*/
	...
	@return un par donde el segundo campo vale true si la entrada se ha podido insertar con éxito, esto es, no existe una
	mutación con igual valor en el conjunto. False en caso contrario.
	...
	*/
	...
	@return devuelve la posicion siguiente al elemento borrado
	...
	*/
	...
	@return devuelve el numero de elementos borrados
	@post Si esta en el conjunto su tamaño se decrementa en 1.
	*/
	...
	*/
	...
	@param[in] val entrada. Devuelve un iterador al primer elemento que cumple que "elemento>e", esto es, el primer
	elemento ESTRICTAMENTE mayor que val
